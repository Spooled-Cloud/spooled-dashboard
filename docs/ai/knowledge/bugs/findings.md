...
| DB-24 | P2  | Overview trend chart labeled unhealthy workers Idle (`workers.unhealthy` → `worker_status.idle`) | `src/lib/hooks/useDashboard.ts`; backend `DashboardData.workers` is healthy/unhealthy | **FIXED** |
